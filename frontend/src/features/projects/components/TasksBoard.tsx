...
function TaskCard({ task, dragging }: { task: Task; dragging?: boolean }) {
	...
	return (
		...
		>
			{/* Top Row: Drag Handle, Title, Kebab Menu */}
			...
			{/* Metadata Row: Tag and Priority Grouped Together */}
			<div className="task-card-meta-row">
				<TagChip label={task.label} variant={task.labelChip} />
				<span className="task-card-priority-indicator">
					<span
						className="task-card-priority-dot"
						style={{ background: PRIORITY_COLOR[task.priority] }}
					/>
					<span className="task-card-priority-text" style={{ color: PRIORITY_COLOR[task.priority] }}>
						{task.priority}
					</span>
				</span>
			</div>

			{/* Blocked Note (if applicable) */}
			{/* {task.blockedNote && (
				<div className="task-card-blocked-note">{task.blockedNote}</div>
			)} */}

			{task.blockedNote && (
				<div className="task-card-blocker-banner">
					<AlertCircle size={14} className="blocker-icon" />
					<span>{task.blockedNote}</span>
				</div>
			)}

			{/* Footer Row: Avatar, Done Badge, and Progress */}
			<div className="task-card-footer">
				<div className="task-card-footer-left">
					{/* <div className="pm-deadline-owner">
						<span className="pm-deadline-avatar" style={{ background: task.assignee.accent }}>
							{task.assignee.initials}
						</span>
						<span className="pm-deadline-name">{task.assignee.name}</span>
					</div> */}
					<div className="pm-deadline-owner">
						<span className="pm-deadline-avatar" style={{ background: task.assignee.accent }}>
							{task.assignee.initials}
				...
				{/* Progress moved into the footer, aligned right */}
